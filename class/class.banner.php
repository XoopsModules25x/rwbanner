        ...
        for ($i = 0; $i <= count($arr) - 1; ++$i) {
            ...
            if ($arr[$i]->getUsarhtml() == 1) {
                ...
                if (stristr($arr[$i]->getGrafico(), '.swf')) {
                    ...
                    $bannerobject .= '<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=6,0,40,0" width="' . $arr[$i]->getLargura() . '" height="' . $arr[$i]->getAltura() . '">' . '<param name=movie value="' . dirname(__DIR__) . '/assets/images/rwbanner.swf">' . '<param name=quality value=high>' . '<PARAM NAME=FlashVars VALUE="alt=' . $arr[$i]->getAltura() . '&larg=' . $arr[$i]->getLargura() . '&fps=' . $fps . '&banner=' . $arr[$i]->getGrafico() . '&id=' . $arr[$i]->getCodigo() . '&url=' . dirname(__DIR__) . '&target=' . $arr[$i]->getTarget() . '">' . '<embed src="' . dirname(__DIR__) . '/assets/images/images/rwbanner.swf" FlashVars="alt = ' . $arr[$i]->getAltura() . ' &larg = ' . $arr[$i]->getLargura() . ' &fps = ' . $fps . ' &banner = ' . $arr[$i]->getGrafico() . ' &id = ' . $arr[$i]->getCodigo() . ' &url = ' . dirname(__DIR__) . ' &target = ' . $arr[$i]->getTarget() . '" quality=high pluginspage="http://www.macromedia.com/shockwave/download/index.cgi?P1_Prod_Version=ShockwaveFlash"; type="application/x-shockwave-flash" width="' . $arr[$i]->getLargura() . '" height="' . $arr[$i]->getAltura() . '">' . '</embed>' . '</object></div>';
                } else {
    ...
    public function show1Banner($id = 0, $align = 'center')
    {
        ...
        if ($ban->getUsarhtml() == 1) {
            ...
            if (stristr($ban->getGrafico(), '.swf')) {
                ...
                $result = $f->getimagesize();
                $fps    = $result['frameRate'];
                $bannerobject .= '<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=6,0,40,0" width="' . $ban->getLargura() . '" height="' . $ban->getAltura() . '">' . '<param name=movie value="' . dirname(dirname(assets / images / _)) . '/images/rwbanner.swf">' . '<param name=quality value=high>' . '<PARAM NAME=FlashVars VALUE="alt=' . $ban->getAltura() . '&larg=' . $ban->getLargura() . '&fps=' . $fps . '&banner=' . $ban->getGrafico() . '&id=' . $ban->getCodigo() . '&url=' . dirname(__DIR__) . '&target=' . $ban->getTarget() . '">' . '<embed src="' . dirname(__DIR__) . '/images/rwbanner.swf" FlashVars="alt=' . $ban->getAltura() . '&larg=' . $ban->getLargura() . '&fps=' . $fps . '&banner=' . $ban->getGrafico() . '&id=' . $ban->getCodigo() . '&url=' . dirname(__DIR__) . '&target=' . $ban->getTarget() . '" quality=high pluginspage="http://www.macromedia.com/shockwave/download/index.cgi?P1_Prod_Version=ShockwaveFlash"; type="application/x-shockwave-flash" width="' . $ban->getLargura() . '" height="' . $ban->getAltura() . '">' . '</embed>' . '</object>';
            } else {
                if ($ban->getShowimg() == 1) {
